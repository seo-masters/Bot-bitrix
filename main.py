...
while True:
    ...
    # Cambia el enfoque a la ventana emergente
    driver.switch_to.window(ventanas[-1])

    # Búsqueda del campo para correo
    ...
    # Envío siguiente para volver a la web principal
    buttonNext = driver.find_element(By.CLASS_NAME, "b24-network-auth-form-btn")
    buttonNext.click()
    time.sleep(5)
    ...
    # Diccionario de empresas y sus IDs de chat en Skype
    lista_empresas = {
    ...
    for empresa in empresas:
        try:
            if empresa in lista_empresas:
                ...
                # Busca y filtra registros en Bitrix24
                ...
                try:
                    ...
                    while llamadaDesconocida:
                        ...
                        if resultados == []:
                            try:
                                cursor.execute(
                                    "INSERT INTO leads (employee, phone, call_date, status, day) VALUES (?, ?, ?, ?, ?)",
                                    (datos_filtrados["columna_2"], datos_filtrados["columna_4"], datos_filtrados["columna_7"], datos_filtrados["columna_8"], fecha_formateada))
